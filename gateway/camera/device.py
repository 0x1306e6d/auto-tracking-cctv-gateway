...
class CameraDevice(object):
    def __init__(self, stream, address):
        ...
        #self.face_image = face_recognition.load_image_file("/home/heost/auto-tracking-cctv-gateway/gateway/images/Seung-tae/heo-1.jpg")
        #self.face_image = face_recognition.load_image_file("/home/heost/auto-tracking-cctv-gateway/gateway/images/So-eun/kwon-3.jpg")
        self.face_image = face_recognition.load_image_file("./images/So-eun/kwon-3.jpg")
        self.face_encoding = face_recognition.face_encodings(self.face_image)[0]
        #self.face_encoding = cv2.cvtColor(self.face_encoding,cv2.COLOR_RGB2GRAY)
    ...
    def display_face_recognition(self, frame):
        ...
        for (top_, right_, bottom_, left_), name in zip(self.face_locations, self.face_names):
            ...
            cv2.rectangle(display_image, (left_, top_), (right_, bottom_), (0, 0, 255), 2)
            #cv2.rectangle(display_image, (left_, bottom_ - 35), (right_, bottom_), (0, 0, 255), cv2.FILLED)
            font = cv2.FONT_HERSHEY_DUPLEX
            cv2.putText(display_image, name, (left_ + 6, bottom_ - 6), font, 1.0, (255, 255, 255), 1)
    ...
    def getCenterpoint(self, frame) :
      ...
      cv2.accumulateWeighted(color_image, self.running_average_image, 0.320,None)

      # Convert the scale of the moving average.
      self.running_average_in_display_color_depth = cv2.convertScaleAbs(self.running_average_image)

      # Subtract the current frame from the moving average.
      cv2.absdiff(color_image,self.running_average_in_display_color_depth,self.difference)

      # Convert the image to greyscale.
      self.grey_image = cv2.cvtColor(self.difference, cv2.COLOR_BGR2GRAY)

      # Threshold the image to a black and white motion mask:
      ret, self.grey_image = cv2.threshold(self.grey_image, 2, 255, cv2.THRESH_BINARY)

      # Smooth and threshold again to eliminate "sparkles"
      self.grey_image = cv2.GaussianBlur(self.grey_image, (19,19), 0)
      ret, self.grey_image = cv2.threshold(self.grey_image, 240, 255, cv2.THRESH_BINARY)
      ...
      _, contour, hierarchy = cv2.findContours( self.grey_image, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE )
      ...
      for entity in bounding_box_list:
         if (int(entity[1][0] - entity[0][0]) > 50) & (int(entity[1][1] - entity[0][1]) > 50) :
            center_point = int((entity[0][0]+entity[1][0])/2), int((entity[0][1] + entity[1][1])/2)
            cv2.circle(self.display_image, center_point, 20, (0,0,255), 1)
            # cv2.circle(self.display_image, center_point, 15, (0,255,0), 1)
            cv2.circle(self.display_image, center_point, 10, (255,0,0), 2)
